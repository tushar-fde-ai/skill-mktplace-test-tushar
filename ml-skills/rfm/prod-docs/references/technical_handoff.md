...
Collect all information needed to generate the technical hand-off documentation for the RFM Customer Segmentation model.
...
- **RFM sub-folder page ID** — you'll use this as the `parentId` when creating documentation pages

The final page hierarchy will be:
```
[Customer Folder]
└── FDE Solutions (or ML & Analytics Projects, etc.)
    └── RFM Customer Segmentation          ← parentId for documentation pages
        ├── RFM Configuration Summary
        ├── RFM Architecture & Output Schema
        └── RFM Runbook & Maintenance
```
...
Ask: **What is the name of the database where the RFM tables live?**
